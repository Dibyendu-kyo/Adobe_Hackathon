#!/usr/bin/env python3
"""
Standalone script for Round 1B processing
...
def main():
    if len(sys.argv) < 5:
        print(json.dumps({"error": "Usage: python process_round1b.py <models_dir> <persona> <job_to_be_done> <pdf_file1> [pdf_file2] ..."}), file=sys.stderr)
        sys.exit(1)
    ...
    try:
        ...
        for pdf_path in pdf_files:
            ...
            # Take top 3 chunks from each document instead of just 1
            # This gives us more variety and better coverage
            for i, chunk in enumerate(ranked[:3]):
                chunk["document"] = os.path.basename(pdf_path)
                chunk["doc_rank"] = i + 1  # Track ranking within document
        ...
        # Enhanced filtering and ranking with more aggressive filtering
        filtered_chunks = []
        for chunk in all_chunks:
            section_title = chunk.get("section_title", "").lower()
            content = chunk.get("content", "").lower()
            
            # More aggressive filtering of generic sections
            skip_section = False
            
            # Skip purely generic sections
            generic_terms = ["introduction", "conclusion", "overview", "preface", "summary", "about this guide"]
            if any(generic in section_title for generic in generic_terms):
                # Only keep if content is highly specific and actionable
                actionable_keywords = ["specific restaurant", "hotel recommendation", "activity list", "step-by-step", "detailed guide", "practical tips"]
                if not any(actionable in content for actionable in actionable_keywords):
                    skip_section = True
            
            # Skip if section title is too short or generic
            if len(section_title.strip()) < 10 or section_title.strip() in ["introduction", "conclusion", "overview"]:
                skip_section = True
            ...
            if persona.lower() == "travel planner":
                # High boost for specific travel planning content
                title_boost = 0
                content_boost = 0
        ...
        # Sort by adjusted score and take top 5, ensuring diversity
